...
from Models.CNN_BiLSTM import CNNLSTM as CNNBiLSTM, train_cnnlstm as train_cnn_bilstm, evaluate_cnnlstm as evaluate_cnn_bilstm
...
dp.set_seed(42)  # Fix random state for reproducibility
...
def objective(trial):
    ...
    train_kwargs = {
        ...
        "best_model_path": f"studies/best_temp_CNNBiLSTM_trial_{trial.number}.pt"
    }

    train_sig = inspect.signature(train_cnn_bilstm)
    if "class_weights" in train_sig.parameters:
        train_kwargs["class_weights"] = class_weights

    history = train_cnn_bilstm(**train_kwargs)
    ...
    eval_result = evaluate_cnn_bilstm(
        model=model,
...
def save_best_model_callback(study, trial):
    # If this trial is the best so far
    if study.best_trial.number == trial.number:
        src = f"studies/best_temp_CNNBiLSTM_trial_{trial.number}.pt"
        if os.path.exists(src):
            optuna_dir = Path("studies/optuna_optimize_CNN_BiLSTM")
...
if __name__ == "__main__":
    optuna_dir = Path("studies/optuna_optimize_CNN_BiLSTM")
    optuna_dir.mkdir(parents=True, exist_ok=True)
    # Separate db from the old study.db: those trials were scored on the test split under
    # a session-level split, so their values are not comparable with the ones below.
    db_path = f"sqlite:///{optuna_dir}/study_subject_independent.db"

    study = optuna.create_study(direction="maximize", study_name="optimize_CNN_BiLSTM_subject_independent", storage=db_path, load_if_exists=True)
    
    # Ensure trial temp files are cleared properly during optimization
    try:
        study.optimize(objective, n_trials=20, callbacks=[save_best_model_callback])
    finally:
        # Clean up temporary trial checkpints
        for f in Path("studies").glob("best_temp_CNNBiLSTM_trial_*.pt"):
            f.unlink()
    
    print("\n===============================")
    print("Best parameters for CNN_BiLSTM:")
    ...
    val_result = evaluate_cnn_bilstm(
        model=model,
        data_loader=splits.val_loader,
        device=device,
        history=history,
    ...
    test_result = evaluate_cnn_bilstm(
        model=model,
