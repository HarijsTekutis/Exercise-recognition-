...
from Models.CNN_BiGRU import CNNBiGRU, train_cnn_bigru, evaluate_cnn_bigru
...
def objective(trial):
    ...
    model = CNNBiGRU(
        ...
        hidden_size=64, # Default from main 
    ...
    train_kwargs = {
        ...
        "best_model_path": f"studies/best_temp_CNNBiGRU_trial_{trial.number}.pt"
    }

    train_sig = inspect.signature(train_cnn_bigru)
    if "class_weights" in train_sig.parameters:
        train_kwargs["class_weights"] = class_weights

    # Train model
    history = train_cnn_bigru(**train_kwargs)
...
def save_best_model_callback(study, trial):
    if study.best_trial.number == trial.number:
        src = f"studies/best_temp_CNNBiGRU_trial_{trial.number}.pt"
...
if __name__ == "__main__":
    ...
    try:
        ...
        for f in Path("studies").glob("best_temp_CNNBiGRU_trial_*.pt"):
            f.unlink()
