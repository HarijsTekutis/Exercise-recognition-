...
def objective(trial):
    ...
    train_kwargs = {
        ...
        "best_model_path": f"studies/best_temp_CNNResBiLSTM_trial_{trial.number}.pt"
    }

    train_sig = inspect.signature(train_cnn_resbilstm)
    if "class_weights" in train_sig.parameters:
        train_kwargs["class_weights"] = class_weights

    history = train_cnn_resbilstm(**train_kwargs)
    ...
    eval_result = evaluate_cnn_resbilstm(
        model=model,
...
def save_best_model_callback(study, trial):
    if study.best_trial.number == trial.number:
        src = f"studies/best_temp_CNNResBiLSTM_trial_{trial.number}.pt"
...
if __name__ == "__main__":
    ...
    try:
        study.optimize(
            objective,
            n_trials=len(SEARCH_SPACE["batch_size"]) * len(SEARCH_SPACE["lstm_layers"]),
            n_jobs=N_PARALLEL_TRIALS,
            callbacks=[save_best_model_callback],
        )
    finally:
        for f in Path("studies").glob("best_temp_CNNResBiLSTM_trial_*.pt"):
            f.unlink()
    
    print("\n===============================")
    print("Best parameters for CNN_ResBiLSTM:")
    ...
    val_result = evaluate_cnn_resbilstm(
        model=model,
        data_loader=splits.val_loader,
        device=device,
        history=history,
    ...
    test_result = evaluate_cnn_resbilstm(
        model=model,
