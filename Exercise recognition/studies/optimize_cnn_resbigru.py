...
from Models.CNN_ResBiGru import CNNResBiGRU, train_cnn_resbigru, evaluate_cnn_resbigru
...
def objective(trial):
    ...
    gru_layers = trial.suggest_int("gru_layers", 2, 6)
    ...
    train_kwargs = {
        ...
        "best_model_path": f"studies/best_temp_CNNResBiGRU_trial_{trial.number}.pt"
    }

    train_sig = inspect.signature(train_cnn_resbigru)
    if "class_weights" in train_sig.parameters:
        train_kwargs["class_weights"] = class_weights

    history = train_cnn_resbigru(**train_kwargs)
    ...
    eval_result = evaluate_cnn_resbigru(
        model=model,
...
def save_best_model_callback(study, trial):
    if study.best_trial.number == trial.number:
        src = f"studies/best_temp_CNNResBiGRU_trial_{trial.number}.pt"
        if os.path.exists(src):
            optuna_dir = Path("studies/optuna_optimize_CNN_ResBiGRU")
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(direction="maximize", study_name="optimize_CNN_ResBiGRU_subject_independent", storage=db_path, load_if_exists=True)
    
    try:
        study.optimize(objective, n_trials=15, callbacks=[save_best_model_callback])
    finally:
        for f in Path("studies").glob("best_temp_CNNResBiGRU_trial_*.pt"):
            f.unlink()
    
    print("\n===============================")
    print("Best parameters for CNN_ResBiGRU:")
    ...
    val_result = evaluate_cnn_resbigru(
        model=model,
        data_loader=splits.val_loader,
        device=device,
        history=history,
    ...
    test_result = evaluate_cnn_resbigru(
        model=model,
