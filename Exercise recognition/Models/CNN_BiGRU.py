...
class Model_1D_CNN_BiGRU(nn.Module):
	def __init__(self, num_classes: int = 6, num_features: int = 9, hidden_size: int = 64, gru_layers: int = 2):
		...
		self.bigru_blocks.append(BiGRUBlock(input_size=128, hidden_size=hidden_size))
		for _ in range(1, gru_layers):
			self.bigru_blocks.append(BiGRUBlock(input_size=hidden_size * 2, hidden_size=hidden_size))
	...
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		...
		# (batch, time, channels)
		x = x.permute(0, 2, 1)
		for bigru in self.bigru_blocks:
			x = bigru(x)
...
CNNBiGRU = Model_1D_CNN_BiGRU


def train_cnn_bigru(
	model: Model_1D_CNN_BiGRU,
	train_loader,
	val_loader,
	...
	patience: int = 7,
	best_model_path: str = "CNN_BiGRU.pt",
) -> Dict[str, List[float]]:
...
def evaluate_cnn_bigru(
	...
	"""Evaluate CNN-BiGRU on a loader and return standard metrics."""
